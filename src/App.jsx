...
export default function App() {
  ...
  // NEW: number of questions to ask
  ...
  // NEW: available question count based on chapters
  const availableCount = filtered.length;

  // Build quiz when started (shuffle then take first N)
  const quiz = useMemo(() => {
    if (!quizStarted) return [];
    let base = filtered.slice();
    if (shuffleQuestions) base = shuffleArray(base);

    const n = Math.max(1, Math.min(questionCount, base.length));
  ...
  if (!quizStarted) {
    ...
    // Suggest some counts; also allow "All"
    const suggestedCounts = [5, 10, 15, 20, 30, 50].filter((n) => n <= Math.max(availableCount, 50));
    const maxSelectable = Math.max(1, availableCount || 1);
        ...
        <div className="container">
          <header className="topbar" style={{ position: "relative" }}>
            <div className="topbar-left">
              ...
              <div className="subtle">Select chapters and number of questions, then start.</div>
            </div>
          </header>

          <section className="card">
            <h3 className="title">Quiz setup</h3>
            ...
            <div style={{ marginTop: 12 }}>
              <div className="tiny muted" style={{ marginBottom: 6 }}>
                Available in selected chapters: <b>{availableCount}</b>
              </div>

              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                <div className="tiny muted" style={{ minWidth: 120 }}>
                  Number of questions
                </div>

                <select
                  value={Math.min(questionCount, maxSelectable)}
                ...
                >
                  ...
                  <option value={maxSelectable}>All ({maxSelectable})</option>
                </select>

                <button className="btn" onClick={() => setQuestionCount(maxSelectable)} disabled={availableCount === 0}>
                  All
                </button>
              </div>
            </div>
            ...
            <div className="nav" style={{ gridTemplateColumns: "1fr 1fr 1fr" }}>
              ...
              <button
                ...
                onClick={() => {
                  // clamp questionCount to available on start
                  const clamped = Math.max(1, Math.min(questionCount, Math.max(1, availableCount)));
                  setQuestionCount(clamped);
                  setQuizStarted(true);
                }}
                disabled={!selectedChapters.length || availableCount === 0}
              >
                Start quiz
              </button>
            </div>
              ...
              {quiz.map((q, i) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      // jump to that question in the MAIN quiz (not retry)
                      setRetryMode(false);
                      setRetryQuiz([]);
                      setIndex(i);
                      window.scrollTo({ top: 0, behavior: "smooth" });
